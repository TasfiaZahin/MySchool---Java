 ...
 * @author User
 */
public class MIT_READController implements Initializable {
    @FXML
    ...
    private void handleButtonAction(ActionEvent event) throws IOException
    {
        ...
        {
            MySchool.window.setScene(MySchool.MIT_CSE_READ);
        }
        else if(event.getSource()==button_mit_eee)
        {
            MySchool.window.setScene(MySchool.MIT_EEE_READ);
        }
        else if(event.getSource()==button_mit_me)
        {
            MySchool.window.setScene(MySchool.MIT_ME_READ);
        }
