 ...
 * @author User
 */
public class OXFORD_READController implements Initializable {
    @FXML
    ...
    private void handleButtonAction(ActionEvent event) throws IOException
    {
        ...
        {
            MySchool.window.setScene(MySchool.OXFORD_CSE_READ);
        }
        else if(event.getSource()==button_oxford_eee)
        {
            MySchool.window.setScene(MySchool.OXFORD_EEE_READ);
        }
        else if(event.getSource()==button_oxford_me)
        {
            MySchool.window.setScene(MySchool.OXFORD_ME_READ);
        }
